...
export default function SignUpPage() {
  ...
  // Redirect to /login if Clerk is not enabled
  useEffect(() => {
    if (!CLERK_ENABLED || !SignUp) {
      router.replace('/login')
    }
  }, [router])

  // If Clerk is not enabled, show loading/redirect message
  if (!CLERK_ENABLED || !SignUp) {
    return (
  ...
  // Render Clerk SignUp component only if Clerk is enabled
  return (
    <div className="min-h-screen bg-[#07090a] relative overflow-hidden flex items-center justify-center">
      {/* Background Effects */}
